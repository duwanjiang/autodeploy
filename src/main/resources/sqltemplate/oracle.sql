...
#sql("querySystemByCode")
 select systemCode,systemName from tbl_paramSystemConfig where isActive = 1 and systemCode = #para(systemCode) and rownum <= 1
#end

#查询系统的第一个收费站
#sql("queryFirstStationByCode")
 select * from tbl_paramSystemConfig where isActive = 1 and systemCode = #para(systemCode) and rownum <= 1
#end